...
import type { LoanStatus, LoanApplicationDetail } from "@/types/loan";
import { fetchApplicationDetail } from "@/api/applications.api";
import { ArrowLeft, Clock, CheckCircle2, Circle, Check, X } from "lucide-react";
...
const ApplicationDetailPage = () => {
  ...
  useEffect(() => {
    if (!id) return;
    fetchApplicationDetail(id).then((res) => {
  ...
  if (!app) {
    return <div className="p-6 text-muted-foreground">Loading application…</div>;
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-muted-foreground">{app.customerName} &middot; {app.loanType} &middot; {app.amount}</p>
        </div>
      </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid gap-3 sm:grid-cols-2 text-sm">
                <div><span className="text-muted-foreground">Customer:</span> <span className="font-medium">{app.customerName}</span></div>
                <div><span className="text-muted-foreground">Loan Type:</span> <span className="font-medium">{app.loanType}</span></div>
                ...
                <div><span className="text-muted-foreground">RM:</span> <span className="font-medium">{app.rmName}</span></div>
            ...
            <CardContent className="space-y-2">
              {app.documents.map((doc) => (
                <div key={doc.name} className="flex items-center gap-2 text-sm">
                  {doc.uploaded ? <Check className="h-4 w-4 text-success shrink-0" /> : <X className="h-4 w-4 text-destructive shrink-0" />}
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardContent>
              ...
              <div className="mt-3 text-xs text-muted-foreground">Created: {app.createdAt}</div>
            </CardContent>
          </Card>
